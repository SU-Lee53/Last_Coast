...
UENUM(BlueprintType)
enum class EMeshExportFormat : uint8
{
    FBX  UMETA(DisplayName = "FBX"),
    GLTF UMETA(DisplayName = "glTF (.gltf)"),
    GLB  UMETA(DisplayName = "GLB (.glb)")
};
...
public:
    ...
    UFUNCTION(BlueprintCallable, Category = "JSON Save System")
    static bool SaveCheckpointsToJson(const TArray<AActor*>& Actors, const FString& FileName);
...
private:
    ...
    static bool ExportMesh(UStaticMesh* Mesh, const FString& FileName, EMeshExportFormat ExportFormat, bool bShowOptions = false);

    static FString GetMeshExportExtension(EMeshExportFormat ExportFormat);
    static FString GetMeshExportFormatName(EMeshExportFormat ExportFormat);
    ...
    // MappingScale + CustomUVType + 랜드스케이프 치수 -> 클라이언트 Tiling(= cm당 텍스처 반복)
