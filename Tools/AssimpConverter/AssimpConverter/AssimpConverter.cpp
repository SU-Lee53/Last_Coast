...
void AssimpConverter::GatherTransformedPositions(const aiNode* pNode, const XMMATRIX& xmmtxParentAccum, std::vector<XMFLOAT3>& outPositions) const
{
	XMFLOAT4X4 xmf4x4Local = aiMatrixToXMMatrix(pNode->mTransformation);
	XMFLOAT4X4 xmf4x4Engine = ConvertMatrixToEngine(xmf4x4Local);
	XMMATRIX xmmtxAccum = XMLoadFloat4x4(&xmf4x4Engine) * xmmtxParentAccum;
	...
	for (int i = 0; i < pNode->mNumMeshes; ++i) {
		...
		for (int v = 0; v < pMesh->mNumVertices; ++v) {
			XMFLOAT3 xmf3Pos = aiVector3DToXMVector(pMesh->mVertices[v]);
			XMVECTOR xmvPos = XMVectorScale(XMLoadFloat3(&xmf3Pos), fScale);
			xmvPos = XMVector3TransformCoord(xmvPos, xmmtxSceneToEngine);
			xmvPos = XMVector3TransformCoord(xmvPos, xmmtxAccum);
			XMStoreFloat3(&xmf3Pos, xmvPos);
			outPositions.push_back(xmf3Pos);
		}
	}

	for (int i = 0; i < pNode->mNumChildren; ++i) {
		GatherTransformedPositions(pNode->mChildren[i], xmmtxAccum, outPositions);
	}
}
...
void AssimpConverter::SerializeModel(const std::string& strPath, const std::string& strName)
{
	...
	// Merged bounds (all meshes in hierarchy, transformed to model root space)
	std::vector<XMFLOAT3> xmf3AllPositions;
	GatherTransformedPositions(m_pRootNode, XMMatrixIdentity(), xmf3AllPositions);
	if (!xmf3AllPositions.empty()) {
		BoundingBox xmMergedAABB;
		BoundingBox::CreateFromPoints(xmMergedAABB, xmf3AllPositions.size(), xmf3AllPositions.data(), sizeof(XMFLOAT3));
		hierarchyJson["MergedBounds"]["Center"] = { xmMergedAABB.Center.x, xmMergedAABB.Center.y, xmMergedAABB.Center.z };
		hierarchyJson["MergedBounds"]["Extents"] = { xmMergedAABB.Extents.x, xmMergedAABB.Extents.y, xmMergedAABB.Extents.z };
		DisplayText("MergedBounds: Center(%.1f, %.1f, %.1f) Extents(%.1f, %.1f, %.1f)\r\n",
			xmMergedAABB.Center.x, xmMergedAABB.Center.y, xmMergedAABB.Center.z,
			xmMergedAABB.Extents.x, xmMergedAABB.Extents.y, xmMergedAABB.Extents.z);
	}
	...
	std::ofstream outBinary{ strSave, std::ios::binary };
	std::vector<uint8_t> bson = nlohmann::json::to_bson(hierarchyJson);
	outBinary.write(reinterpret_cast<const char*>(bson.data()), bson.size());
