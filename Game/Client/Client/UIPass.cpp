...
void UIPass::OnPreRender(ComPtr<ID3D12GraphicsCommandList> pd3dCommandList, const RenderPassInput& input, OUT RenderPassOutput& output, OUT DescriptorHandle& outDescHandle)
{
	...
	const UIBoard::UIComponentLayer& UILayers = CUR_SCENE->GetUIBoard()->GetUILayers();
	...
	for (int i = nLayers - 1; i >= 0; --i) {
		const auto& UIInLayer = UILayers[i];

		for (auto pUIComponent : UIInLayer) {
			if (pUIComponent->IsVisible() == false) {
				continue;
			}
			UIRectData UIData = pUIComponent->MakeSBData();
			const auto& texRef = pUIComponent->GetTextureRef();
			if (texRef.IsValid()) {
				auto [id, bInserted] = m_CachedData.textureMap.Insert(texRef.GetID(), &texRef);
				UIData.nTexIndex = static_cast<float>(id + 1);	// 0번에 Atlas 가 보관되므로 +1 index 에 원하는 Texture 가 바인딩됨
			}
			m_CachedData.uiDatas[i].push_back(UIData);
		}
	}
...
void UIPass::Render(ComPtr<ID3D12GraphicsCommandList> pd3dCommandList, const RenderPassInput& input, OUT RenderPassOutput& output, OUT DescriptorHandle& outDescHandle)
{
	...
	for (int i = nLayers - 1; i >= 0; --i) {
		//const auto& textsInLayer = m_CachedData.uiDatas[i];
		const auto& UIInLayer = m_CachedData.uiDatas[i];

		// Draw sprite
		if (UIInLayer.size() != 0) {
			auto spriteSBuffer = RENDER->AllocSBuffer<UIRectData>(UIInLayer.size());
			spriteSBuffer.WriteData(UIInLayer);
			pd3dCommandList->SetGraphicsRootShaderResourceView(rootParamUIData, spriteSBuffer.GPUAddress);

			pQuad->Render(pd3dCommandList, UIInLayer.size());
		}
	}
}
...
void UIPass::CreatePipelineState()
{
	...
	HRESULT hr = DEVICE->CreateGraphicsPipelineState(&d3dPipelineDesc, IID_PPV_ARGS(m_pd3dSpritePipelineState.GetAddressOf()));
	if (FAILED(hr)) {
		__debugbreak();
	}
